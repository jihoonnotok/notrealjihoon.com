...
public class loginService
{
	...
	public String login(String userName,String userPw,int userType)
	{
		...
		if(userType==2)
		{
			String sql="from TSonghuoyuan where loginname=? and loginpw=? and del='no'";
			Object[] con={userName,userPw};
			List songhuoyuanList=songhuoyuanDAO.getHibernateTemplate().find(sql,con);
			
			if(songhuoyuanList.size()==0)
			{
				 result="no";
			}
			else
			{
				 WebContext ctx = WebContextFactory.get(); 
				 HttpSession session=ctx.getSession(); 
				 TSonghuoyuan songhuoyuan=(TSonghuoyuan)songhuoyuanList.get(0);
